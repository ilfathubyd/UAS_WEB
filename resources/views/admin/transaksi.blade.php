...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            <br>
            <!-- Main Content -->
            <div class="container bg-info pt-3 pb-3 mt-1 rounded w-75">
                <h4 class="text-center text-light">Tabel Transaksi</h4>
                 <a href="/adminTransaksi" class="btn btn-primary mb-3"><i class="fas fa-user-plus"></i> Tambah</a>
                    <table id="tabel" class="table table-bordered table-striped table-light pt-3">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $row->product->namaProduk }}</td>
                                <td>{{ $row->product->harga }}</td>
                                <td>{{ $row->quantity }}</td>
                                <td class="total-item">Rp. {{$row->product->harga *$row->quantity}}</td>
                                <td>{{ date('d/M/Y', strtotime( $row->tanggal )) }}</td>
                                <td>
                                <a href="{{ url('deleteAdminTransaksi', $row->id) }}" class="btn btn-danger m-1" onclick="del()"><i class="fas fa-trash"></i>  Hapus</a>
                                </td>
                            </tr>
                            @endforeach
                        </tbody>
    ...
    <script>
        function del(){
            var result = confirm('Anda yakin ingin hapus data ini?');
            if (result == false){
            event.preventDefault();
            }
        }
